...
// injected text, frontmatter parsing (block lists, folded scalars, loud failure),
// determinism, every validate-gate tamper case, and classify's risk tiers.
//
...
try {
  ...
  const v0 = run([path.join(TOOLS, 'ce-validate.mjs'), repoA]);
  ...
  async function tamperedCopy(name, mutate) {
    ...
    await mutate(dst);
    return run([path.join(TOOLS, 'ce-validate.mjs'), dst]);
  }
