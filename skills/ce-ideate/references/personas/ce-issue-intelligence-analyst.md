> **Operating constraints (de-plugin-ified persona `ce-issue-intelligence-analyst`).** Read-only: YES — dispatch as subagent_type: Explore. Allowed tools: Read, Grep, Glob, Bash, mcp__github__*. Model: inherit. Stay strictly within these limits; they are not enforced by the runtime once de-plugin-ified.
