> **Operating constraints (de-plugin-ified persona `ce-issue-intelligence-analyst`).** Read-only: YES — prefer a read-only/explore-type subagent if your harness offers one (e.g. Claude Code `Explore`), else a general-purpose subagent with write tools forbidden. Allowed tools: Read, Grep, Glob, Bash, mcp__github__*. Model: inherit. Stay strictly within these limits; they are not enforced by the runtime once de-plugin-ified.
