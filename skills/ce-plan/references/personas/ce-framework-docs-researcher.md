> **Operating constraints (de-plugin-ified persona `ce-framework-docs-researcher`).** Read-only: YES — dispatch as subagent_type: Explore. Allowed tools: Read, Grep, Glob, Bash, WebFetch, WebSearch, mcp__context7__*. Model: inherit. Stay strictly within these limits; they are not enforced by the runtime once de-plugin-ified.
