> **Operating constraints (de-plugin-ified persona `ce-previous-comments-reviewer`).** Read-only: no — dispatch as subagent_type: general-purpose. Allowed tools: Read, Grep, Glob, Bash, Write. Model: inherit. Stay strictly within these limits; they are not enforced by the runtime once de-plugin-ified.
