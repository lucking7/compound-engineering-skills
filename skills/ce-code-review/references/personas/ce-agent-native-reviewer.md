> **Operating constraints (de-plugin-ified persona `ce-agent-native-reviewer`).** Read-only: YES — dispatch as subagent_type: Explore. Allowed tools: Read, Grep, Glob, Bash. Model: inherit. Stay strictly within these limits; they are not enforced by the runtime once de-plugin-ified.
