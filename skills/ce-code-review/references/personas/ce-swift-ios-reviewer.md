> **Operating constraints (de-plugin-ified persona `ce-swift-ios-reviewer`).** Read-only: no — use a general-purpose subagent (or apply inline if your harness has no subagents). Allowed tools: Read, Grep, Glob, Bash, Write. Model: inherit. Stay strictly within these limits; they are not enforced by the runtime once de-plugin-ified.
